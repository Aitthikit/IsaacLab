...
class AnymalDEnvPosNavigate(DirectRLEnv):
    cfg: AnymalDNavigateCfg

    def __init__(self, cfg: AnymalDNavigateCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)

        # Joint position command (deviation from default joint positions)
        ############################################################################################################
        self._raw_actions = torch.zeros(self.num_envs, gym.spaces.flatdim(self.single_action_space), device=self.device)
    ...
    def _setup_scene(self):
        ...
        if not check_file_path(self.cfg.policy_path):
            raise FileNotFoundError(f"Policy file '{self.cfg.policy_path}' does not exist.")
        file_bytes = []
        for i in range(4):
            file_bytes[i] = read_file(self.cfg.policy_path[i])
        self.policy_walk = torch.jit.load(file_bytes[0]).to(self.device).eval()
        self.policy_climbup = torch.jit.load(file_bytes[1]).to(self.device).eval()
        self.policy_climdown = torch.jit.load(file_bytes[2]).to(self.device).eval()
    ...
    ####################################################################################################################
    #need to load all policy and setup each obs of agent in here
    def _pre_physics_step(self, actions: torch.Tensor):
        self._actions = actions.clone()
        if self._actions[0] == 0:
            self._raw_actions = self.policy_climbup(low_level_obs)
        elif self._actions[0] == 1:
            self._raw_actions = self.policy_climdown(low_level_obs)
        else:
            self._raw_actions = self.policy_walk(low_level_obs)
            
        self._processed_actions = self.cfg.action_scale * self._raw_actions + self._robot.data.default_joint_pos
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # print(self._robot.data.body_acc_w)
        # print(base_accel)
        ...
        # print(robot_rpy_mapped)
        # print(f"func : {torch.norm(robot_rpy_mapped,dim=1)}")
        ...
        # print(self._contact_sensor.data.net_forces_w[:, self._feet_ids,:2])
        # print(torch.norm(self._contact_sensor.data.net_forces_w[:, self._feet_ids,:2],dim=-1))
        ...
        # print(self.cfg.episode_length_s)
        # print((self._commands[:,4] - self.episode_length_buf)/50)
        ...
        # robot_rpy = math_utils.euler_xyz_from_quat(self._robot.data.body_com_quat_w[:,self._base_id].squeeze())
        # print(self._robot.data.heading_w)
        
        # robot_rpy_tensor = torch.stack(robot_rpy)
        # robot_rpy_tensor = robot_rpy_tensor.T
        # robot_rpy_mapped = (robot_rpy_tensor + torch.pi) % (2 * torch.pi) - torch.pi
        # command_map = (self._commands[:,3:6] + torch.pi) % (2 * torch.pi) - torch.pi
        command_map = (self._commands[:,3] + torch.pi) % (2 * torch.pi) - torch.pi
        heading_map = (self._robot.data.heading_w + torch.pi) % (2 * torch.pi) - torch.pi
        # print(torch.abs(self._robot.data.heading_w-command_map))
        ...
        # print(reach_condition)

        # print(self._robot.data.default_joint_pos)
        # print(self._robot.data.joint_pos)
        # print(torch.norm(self._robot.data.joint_pos-self._robot.data.default_joint_pos,dim=1))
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if  isinstance(self.cfg, AnymalDClimbUpEnvPosCfg):
            ...
            self._commands[env_ids,2] = 0.6
            # self._commands[env_ids,3] = torch.rand(1,device=self.device) * 2 * torch.pi
            # self._commands[env_ids,6] = torch.tensor(self.max_episode_length,device=self.device,dtype=torch.float32)
            default_root_state = self._robot.data.default_root_state[env_ids]
            default_root_state[:, :3] += self._terrain.env_origins[env_ids]
        ...
        # self._commands[env_ids,3] = torch.tensor(0.0,device=self.device)
        # self._commands[env_ids,4] = torch.tensor(0.0,device=self.device)
        self._commands[env_ids,3] = torch.rand(1,device=self.device) * 2 * torch.pi
        self._commands[env_ids,4] = torch.tensor(self.max_episode_length,device=self.device,dtype=torch.float32)
