...
def main():
    ...
    obs_indices = infos["observations"]["perception"]
    # input_dim = obs_indices.shape[1]
    input_dim = obs_indices.shape[1:]
    output_dim = encoder_cfg.get("student_output_dim", 8)
    device = "cuda"
    encoder_params = {
                "input_dim": input_dim,
                "output_dim": output_dim,
                "hidden_dims": encoder_cfg.get("student_hidden_dims", [256, 128])
    }
    if encoder_cfg.get("student_type","mlp") == "mlp":
        obs_encoder = encoders.ObsEncoder(**encoder_params).to(device)
        print(f"MLP Encoder Structure: {obs_encoder}")    
    elif encoder_cfg.get("student_type","mlp") == "gru":
        encoder_params.update({
            "gru_hidden_size": encoder_cfg.get("student_gru_hidden_size", 256),
            "gru_num_layers": encoder_cfg.get("student_gru_num_layers", 2)
        })
        obs_encoder = encoders.GRUEncoder(**encoder_params).to(device)
        print(f"GRU Encoder Structure: {obs_encoder}")
    elif encoder_cfg.get("student_type","mlp") == "conv":
        encoder_params.update({
            "conv_channels": encoder_cfg.get("student_conv_channels", [32, 64, 128]),
            "conv_kernel_sizes": encoder_cfg.get("student_conv_kernel_sizes", [3, 3, 3]),
            "conv_strides": encoder_cfg.get("student_conv_strides", [1, 1, 1])
        })
        obs_encoder = encoders.ConvEncoder(**encoder_params).to(device)
        print(f"Conv Encoder Structure: {obs_encoder}")

    elif encoder_cfg.get("student_type","mlp") == "convgru":
        encoder_params.update({
            "conv_channels": encoder_cfg.get("student_conv_channels", [32, 64]),
            "conv_kernel_sizes": encoder_cfg.get("student_conv_kernel_sizes", [3, 3]),
            "pool_sizes": encoder_cfg.get("student_pool_sizes", [2, 2, 2]),
            "gru_hidden_size": encoder_cfg.get("student_gru_hidden_size", 256),
            "gru_num_layers": encoder_cfg.get("student_gru_num_layers", 1)
        })
        obs_encoder = encoders.ConvGRUEncoder(**encoder_params).to(device)
        print(f"ConvGRU Encoder Structure: {obs_encoder}")

    obs_encoder.load_state_dict(torch.load(resume_path)["student_encoder_state_dict"])
    obs_encoder.eval()
    print(f"obs_indic {obs_indices.shape}")
    dummy_obs = torch.zeros(1, 1,45,60, device=agent_cfg.device)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            selected_obs = infos["observations"]["perception"]
            print(f'Selected obs shape:{selected_obs.shape}')
            ...
            actions = policy(modified_obs)
            # actions = policy(obs)
            # print("action",actions)

            # actions = torch.zeros_like(actions)
            # if timestep % 200 >= 100:
            #     actions = actions + -0.5  # random actions for testing
            # actions = actions + ppo_runner.alg.action_space.sample()  # random actions for testing
